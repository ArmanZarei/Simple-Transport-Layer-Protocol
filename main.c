...
/*********************************** LinkedList for packets (sorted by seqnum) ***********************************/
struct PacketNode {
    struct pkt packet;
    struct PacketNode* next;
};
struct PacketList {
  struct PacketNode* first_packet;
  int len;
};

/****** Creates a Packet List *****/
struct PacketList *init_packet_list() {
  struct PacketList *packet_list = (struct PacketList *)malloc(sizeof(struct PacketList));
  packet_list->len = 0;
  packet_list->first_packet = NULL;

  return packet_list;
}

/****** Adds a packet node to the packet list (keeps the list sorted by seqnum) *****/
void add_packet_node(struct PacketList *packet_list, struct pkt packet) {
  if (packet_list->len == 0) {
    struct PacketNode *packet_node = (struct PacketNode *)malloc(sizeof(struct PacketNode));
    packet_node->next = NULL;
    packet_node->packet = packet;

    packet_list->first_packet = packet_node;
  } else {
    struct PacketNode *prev_to_be_inserted = NULL;
    struct PacketNode *next_to_be_inserted = packet_list->first_packet;
    while (next_to_be_inserted != NULL && (next_to_be_inserted->packet).seqnum <= packet.seqnum) {
      if ((next_to_be_inserted->packet).seqnum == packet.seqnum) { // Repeated packet
        return;
      }
      prev_to_be_inserted = next_to_be_inserted;
      next_to_be_inserted = next_to_be_inserted->next;
    }

    struct PacketNode *packet_node = (struct PacketNode *)malloc(sizeof(struct PacketNode));
    packet_node->packet = packet;
    packet_node->next = next_to_be_inserted;

    if (prev_to_be_inserted == NULL) {
      packet_list->first_packet = packet_node;
    } else {
      prev_to_be_inserted->next = packet_node;
    }
  }
...
/****** Removes a all packets before a packet node *****/
int remove_packet_nodes_till(struct PacketList *packet_list, int seqnum) {
  struct PacketNode *packet_node = packet_list->first_packet;
  
  int remove_cnt = 1;
  while (packet_node != NULL && (packet_node->packet).seqnum != seqnum) {
  ...
  packet_list->len -= remove_cnt;
  packet_list->first_packet = packet_node->next;
  
  return remove_cnt;
}

/****** Prints the packet list (for debugging) *****/
void print_packets_window(struct PacketList *packet_list) {
  if (packet_list->len == 0) {
    printf("Packets Windows: [ ]\n");
    return;
  }

  printf("Packets Window: [ ");
  for (struct PacketNode *packet_node = packet_list->first_packet; packet_node != NULL; packet_node = packet_node->next) {
    printf("%d ", (packet_node->packet).seqnum);
  }
  printf("]\n");
}
/****************************************** Server struct *************************************************/
struct Server {
    ...
    struct PacketList *unacked_sent_packets;
    
    struct PacketList *unacked_received_packets;
    int latest_sent_acked_seqnum;
};

struct Server *init_server(int ID) {
  struct Server *server = (struct Server *)malloc(sizeof(struct Server));
  server->ID = ID;
  server->new_seqnum = 1;
  server->unacked_sent_packets = init_packet_list();
  server->unacked_received_packets = init_packet_list();
  server->latest_sent_acked_seqnum = 0;
...
int calc_packet_checksum(struct pkt packet) {
...
bool is_packet_corrupted(struct pkt packet) {
  return packet.checksum != calc_packet_checksum(packet);
}

/******** Create data packet ********/
struct pkt create_data_packet(struct Server *server, struct msg *message) {
  struct pkt packet;
  strcpy(packet.payload, message->data);
  packet.seqnum = server->new_seqnum++;
  packet.acknum = 0;
...
struct pkt create_ack_packet(int acknum) {
  struct pkt ack_pckt;
  ack_pckt.acknum = acknum;
  ack_pckt.seqnum = 0;
  ack_pckt.checksum = calc_packet_checksum(ack_pckt);
...
/******** Server Output ********/
int server_output(message, server) 
  struct msg *message;
  struct Server *server;
{
  if (server->unacked_sent_packets->len == N) {
    return -1;
  }

  struct pkt packet = create_data_packet(server, message);

  add_packet_node(server->unacked_sent_packets, packet);
  
  tolayer3(server->ID, packet);

  if (server->unacked_sent_packets->len == 1) {
    starttimer(server->ID, TIMER_TIME);
  }
...
/******** Server Input ********/
void server_input(packet, server)
  struct pkt packet;
  struct Server *server;
{
  ...
  if (packet.acknum != 0) { // Packet is an ACK packet
    int removed_packet_seqnum = remove_packet_nodes_till(server->unacked_sent_packets, packet.acknum);
    if (removed_packet_seqnum != -1) {
      stoptimer(server->ID);
      if (server->unacked_sent_packets->len != 0) {
        starttimer(server->ID, TIMER_TIME);
      }
    }
  } else { // Packet is not an ACK packet
    if (packet.seqnum > server->latest_sent_acked_seqnum + 8) {
      return;
    }

    if (packet.seqnum <= server->latest_sent_acked_seqnum) { // A packet which already has sent to the application layer
      struct pkt ack_packet = create_ack_packet(server->latest_sent_acked_seqnum);
      tolayer3(server->ID, ack_packet);
      return;
    }

    add_packet_node(server->unacked_received_packets, packet);

    while (server->unacked_received_packets->first_packet != NULL && 
           (server->unacked_received_packets->first_packet->packet).seqnum == server->latest_sent_acked_seqnum+1) {\
      struct PacketNode *current_node = server->unacked_received_packets->first_packet;
      struct pkt current_packet = current_node->packet;

      struct msg message;
      strcpy(message.data, current_packet.payload);
      tolayer5(server->ID, message);

      struct pkt ack_packet = create_ack_packet(current_packet.seqnum);
      tolayer3(server->ID, ack_packet);

      
      server->unacked_received_packets->first_packet = current_node->next;
      free(current_node);

      server->latest_sent_acked_seqnum++;
    }
  }
}

/******** Server Timer Interrupt ********/
void timerinterrupt(server) 
  struct Server *server;
{
  for (struct PacketNode *packet_node = server->unacked_sent_packets->first_packet; packet_node != NULL; packet_node = packet_node->next) {
    tolayer3(server->ID, packet_node->packet);
  }
...
int A_output(message)
  struct msg message;
{
  return server_output(&message, server_A);
}
...
void A_input(packet)
  struct pkt packet;
{
  server_input(packet, server_A);
}

/* called when A's timer goes off */
void A_timerinterrupt()
{
  timerinterrupt(server_A);
}  
...
void B_input(packet)
  struct pkt packet;
{
  server_input(packet, server_B);
}

/* called when B's timer goes off */
void B_timerinterrupt()
{
  timerinterrupt(server_B);
}
...
int B_output(message)  /* need be completed only for extra credit */
  struct msg message;
{
  return server_output(message, server_B);
}
